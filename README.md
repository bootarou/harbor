...
- **harborTalk**（LiveKit）: 入力バー上段に常設。未参加でも参加者が見え、聴講者として参加して
  最大5人まで発言できる。トピック一覧にはライブ中バッジが付き上位に並ぶ。
  設定した環境でのみ有効（構築手順は [VOICE-SETUP.md](VOICE-SETUP.md)）
- **画面共有**: 発言中の参加者が画面/ウィンドウ/タブを共有できる（ルームで同時1人・
  1280x720/10fps・カメラは使わない）。視聴者には強制表示せず「画面を見る」を押したときだけ
  モーダルで開き、全画面表示にも対応。既存の WebRTC 経路を使うため追加のポート開放は不要
...
- `LIVEKIT_API_KEY` / `LIVEKIT_API_SECRET` / `LIVEKIT_WS_URL` / `LIVEKIT_API_URL` — コミュニティのharborTalk（音声）。
