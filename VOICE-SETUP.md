...
画面共有は既存の WebRTC 経路を使うため、**追加のポート開放や設定は不要**です。
音声が通っていれば動きます。

1. 発言中の状態で「🖥 画面共有」→ ブラウザ標準の選択UIで画面/ウィンドウ/タブを選ぶ
2. 別の参加者の画面に「🖥 画面を見る」が出る → 押すとモーダルで表示される
3. モーダルを閉じても共有は続く。再度押せば開き直せる
4. 2人目が「🖥 画面共有」を押すと拒否される（同時に1人まで）
5. ブラウザ標準の「共有を停止」で終了 → 全員から共有中の表示が消える
6. 「🎧 聴講に戻る」または退出で、共有が自動的に終了する
...
画面共有できるのは**発言権を持つ人だけ**です。まず「🎙 発言」で発言権を取得して
ください。ほかの人が共有中の場合もボタンは出ません（同時に1人までのため）。

### 「現在ほかのユーザーが画面を共有しています」と出る

仕様どおりの挙動です。ルーム内で同時に共有できるのは1人までです。
共有者が停止するか、発言権を手放せば共有できるようになります。

### 画面共有が終わったのに「画面を見る」が残る

LiveKit のトラック状態を見て表示しているため、通常は数秒で消えます。
残り続ける場合は共有者側のブラウザが切断を検知できていない可能性があるので、
共有者にページの再読み込みを依頼してください。

### 画面共有の映像が粗い・文字が読みにくい

10fps / 1280x720 / 1.2Mbps に固定しています。動画ではなく資料やコードを
見せる前提の設定です。共有時に「ウィンドウ」や「タブ」を選ぶと、画面全体より
...
発言中の参加者が、自分の画面・ウィンドウ・ブラウザタブを共有できます。
資料やコードを見せながら話す用途を想定しており、動画配信用ではありません。

| 項目 | 値 |
|---|---|
| 取り込み | 1280x720 / 10fps |
| publish | maxFramerate 10 / maxBitrate 1.2Mbps |
| 同時共有 | ルームで**1人まで** |
| カメラ | **使用しない**（画面共有のみ） |

fps を低く抑えているのは、文字の可読性に帯域を回すためです。

**追加のインフラは不要です。** 既存の LiveKit WebRTC 経路（UDP 50000 / 50001、
TCP 7881 / 7882）をそのまま使うため、ポート開放も Cloudflare Tunnel の設定も
増えません。

#### 同時1人をどう保証しているか

**真実の情報源は LiveKit に publish されている ScreenShare トラック**です。
専用のDBテーブルは持ちません。参加者が切断されればトラックも消えるため、
退出・異常切断・ルーム終了のいずれでも**ロックが残留しません**。

そのうえで2段構えにしています。

1. **LiveKit の権限**（本質的な保証）— 共有を許可した相手にだけ
   `canPublishSources` へ `SCREEN_SHARE` を追加します。権限のない参加者は
   LiveKit のレベルで publish できず、UI を改変しても回避できません
2. **インメモリ予約**（競合窓の封鎖）— 「開始要求 → publish 完了」の間はまだ
   トラックが無く、同時押しで両方通る可能性があります。この窓だけを15秒TTLの
   予約で塞ぎ、ルーム単位で直列化しています（presence / ratelimit と同じ
   単一サーバー前提）

#### 発言権との連動

発言権を失うと画面共有も終了します（「聴講に戻る」「ホストによる降格」いずれも）。
権限が LISTENER に戻ることで LiveKit 側もトラックを落とし、あわせてロックも解放します。

#### 視聴は任意

共有が始まっても、他の参加者に映像を強制表示しません。「🖥 画面を見る」を
押したときだけモーダルで開きます。音声だけ聴き続けることもできます。

**見ているかどうかはローカルの状態**で、サーバーには保存しません。
モーダルを閉じても共有者側の配信は止まりません。
